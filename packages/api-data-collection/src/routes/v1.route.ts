...
router.get(
  ...
  asyncHandler<AuthRequest>(async (req, res) => {
    res.status(200).json(getDatasets(req.userId));
  }),
);
...
router.get(
  ...
  asyncHandler<AuthRequest>(async (req, res) => {
    ...
    if (!assertDatasetExists(res, dataset)) return;
...
router.delete(
  "/datasets/:datasetId",
  asyncHandler<AuthRequest>(async (req, res) => {
    const count = deleteDataset(req.userId, req.params.datasetId as string);
